...
#define COMMAND_SCRATCH_SIZE (32 * 1024) // 32k characters is the max that fits in a UNICODE_STRING
...
ProcessMonitor(process_md_t* ctx)
{
    ...
    if (process_info.operation == PROCESS_OPERATION_CREATE) {
        ...
        // We say -1 to esure the last byte stays a 0 for null termination
        ...
        bpf_process_get_image_path(ctx, buffer, COMMAND_SCRATCH_SIZE);
